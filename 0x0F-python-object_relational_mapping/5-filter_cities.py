...
line and performs a search on the database db_name for cities
present in states
...
(SELECT states.id FROM states WHERE states.name = %s LIMIT 1)", (state_name,))
    print(', '.join([cities[0] for cities in cursor.fetchall()]))
    cursor.close()
    db.close()
